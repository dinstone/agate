...
public class AccessLogPlugin extends RoutePlugin {

    public AccessLogPlugin(RouteOptions routeOptions, PluginOptions pluginOptions) {
        super(routeOptions, pluginOptions);
    }

    @Override
    public OperationHandler createHandler(Vertx vertx) {
        JsonObject options = pluginOptions.getOptions();
        if (options != null) {
            String f = options.getString("format", "SHORT").toUpperCase();
            LoggerFormat loggerFormat = LoggerFormat.valueOf(f);
            return new AccessLogHandler(loggerFormat);
        }
